...
old_migration_source = '    "The final migration adds persistent member workflow selections, execution linkage, and workflow-specific context indexing.",'
new_migration_source = '    "It follows `b5d8e1f3a6c9`, adds the active-selection table, connection and command policy fields, execution audit references, and the workflow-scoped context index. Apply all pending migrations and restart the backend before beginning provider-level manual acceptance.",'
if content.count(old_migration_source) != 1:
    raise RuntimeError(f"Expected one migration documentation source line, found {content.count(old_migration_source)}")
