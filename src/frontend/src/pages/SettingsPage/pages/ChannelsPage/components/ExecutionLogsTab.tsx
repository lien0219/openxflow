...
export default function ExecutionLogsTab({ connectionId }: ExecutionLogsTabProps) {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-4 rounded-xl border p-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        />
        <select
          className="primary-input h-10 min-w-[160px] flex-1"
          value={status}
          onChange={(event) => {
      ...
      <div className="min-h-44 min-w-0">
        {isLoading ? (
          ...
          <div className="flex min-h-44 flex-col items-center justify-center gap-3 rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground">
            <span>{copy("运行记录加载失败，请检查后端日志后重试")}</span>
            <Button type="button" size="sm" variant="outline" onClick={() => refetch()}>{copy("重新加载")}</Button>
          </div>
            ...
            <table className="w-full min-w-[1120px] table-fixed text-left text-sm">
              <thead className="border-b text-xs text-muted-foreground">
                <tr>
                  ...
                  <th className="w-36 px-3 py-2">{copy("触发 / 身份")}</th>
                  <th className="w-44 px-3 py-2">{copy("工作流")}</th>
                  <th className="w-52 px-3 py-2">{copy("渠道用户 / 线程")}</th>
                  <th className="w-24 px-3 py-2">{copy("状态")}</th>
                  <th className="w-44 px-3 py-2">{copy("排队 / 执行 / 投递")}</th>
                  <th className="w-16 px-3 py-2">{copy("重试")}</th>
                  <th className="w-64 px-3 py-2">{copy("错误")}</th>
                </tr>
              </thead>
              <tbody>
                {(result?.items ?? []).map((execution) => (
                  <tr key={execution.id} className="border-b align-top last:border-0">
                    <td className="whitespace-nowrap px-3 py-3 text-xs text-muted-foreground">{new Date(execution.created_at).toLocaleString()}</td>
                    <td className="px-3 py-3">
                      <div>{copy(TRIGGER_LABELS[execution.trigger_type])}</div>
                      <div className="mt-1 text-xs text-muted-foreground">
                        {copy(IDENTITY_LABELS[execution.execution_identity_type])}{execution.command_name ? ` · ${execution.command_name}` : ""}
                      </div>
                    </td>
                    <td className="px-3 py-3 font-mono text-xs">
                      <div>{execution.flow_id?.slice(0, 8) ?? copy("工作流已删除")}</div>
                      <div className="mt-1 truncate text-muted-foreground" title={execution.external_event_id}>{execution.external_event_id}</div>
                    </td>
                    <td className="px-3 py-3 font-mono text-xs text-muted-foreground">
                      <div className="truncate" title={execution.external_user_id ?? undefined}>{execution.external_user_id || "-"}</div>
                      <div className="mt-1 truncate" title={execution.session_id ?? undefined}>{execution.session_id || "-"}</div>
                    </td>
                    <td className="px-3 py-3"><span className="rounded-full bg-muted px-2 py-1 text-xs">{copy(STATUS_LABELS[execution.status])}</span></td>
                    ...
                    <td className="px-3 py-3 text-xs text-destructive">
                      {execution.error_code ? <div className="truncate font-mono">{execution.error_code}</div> : null}
                      <div className="line-clamp-3 break-words">{execution.error_message || "-"}</div>
                    </td>
                  </tr>
                ))}
              </tbody>
