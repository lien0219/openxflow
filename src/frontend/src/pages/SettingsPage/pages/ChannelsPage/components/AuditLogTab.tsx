          ...
          <table className="w-full min-w-[1080px] text-left text-sm">
            <thead className="border-b text-xs text-muted-foreground">
              <tr>
                ...
                <th className="px-3 py-2">{copy("资源")}</th>
                <th className="px-3 py-2">{copy("操作者")}</th>
                <th className="px-3 py-2">{copy("变更内容")}</th>
              </tr>
            </thead>
            <tbody>
              {(result?.items ?? []).map((audit) => (
                <tr key={audit.id} className="border-b align-top last:border-0">
                  ...
                  <td className="max-w-[520px] px-3 py-3">
                    <pre className="max-h-52 overflow-auto whitespace-pre-wrap break-words rounded-md bg-muted/50 p-3 text-xs">
