...
export default function MessagesTab({ connectionId }: MessagesTabProps) {
  ...
  return (
    <div className="flex flex-col gap-4 rounded-xl border p-5">
      ...
      <div className="grid gap-3 xl:grid-cols-[minmax(240px,2fr)_1fr_1fr_160px_160px_auto]">
        <Input
          value={queryDraft}
          ...
          <table className="w-full min-w-[1180px] text-left text-sm">
            <thead className="border-b text-xs text-muted-foreground">
              <tr>
                ...
                <th className="px-3 py-2">{copy("方向 / 状态")}</th>
                <th className="px-3 py-2">{copy("发送人 / 用户")}</th>
                <th className="px-3 py-2">{copy("会话 / 线程")}</th>
                <th className="px-3 py-2">{copy("消息内容")}</th>
                <th className="px-3 py-2">{copy("附件")}</th>
