  ...
  return (
    <div className="flex min-w-0 flex-col gap-4 rounded-xl border p-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Input
          className="min-w-[260px] flex-[2_1_360px]"
          value={queryDraft}
          onChange={(event) => setQueryDraft(event.target.value)}
          onKeyDown={(event) => {
            if (event.key === "Enter") applySearch();
          }}
          placeholder={copy("搜索事件 ID、幗台消息 ID 或错误")}
        />
        <select
          className="primary-input h-10 min-w-[170px] flex-1"
          value={status}
          onChange={(event) => {
            ...
            <table className="w-full min-w-[980px] table-fixed text-left text-sm">
              <thead className="border-b text-xs text-muted-foreground">
                <tr>
                  ...
                  <th className="w-32 px-3 py-2">{copy("类型 / 状态")}</th>
                  <th className="w-56 px-3 py-2">{copy("事件 ID")}</th>
                  <th className="w-56 px-3 py-2">{copy("平台消息 ID")}</th>
                  <th className="w-20 px-3 py-2">{copy("尝试次数")}</th>
                  <th className="w-64 px-3 py-2">{copy("最后错误")}</th>
                  <th className="w-28 px-3 py-2">{copy("操作")}</th>
                </tr>
              </thead>
              <tbody>
                {(result?.items ?? []).map((delivery) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3">
                      <div className="truncate">{delivery.delivery_kind}</div>
