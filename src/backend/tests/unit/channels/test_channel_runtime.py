...
async def test_channel_runtime_returns_stream_webhook_and_retry_configuration(monkeypatch) -> None:
    ...
    monkeypatch.setenv("LANGFLOW_CHANNEL_WEBHOOK_JOB_CLEANUP_INTERVAL_SECONDS", "20")
    ...
    assert result.durable_webhook_jobs.cleanup_interval_seconds == 20
    ...
    assert result.durable_webhook_jobs.processing_jobs >= 0
    assert result.durable_webhook_jobs.completed_jobs >= 0
    assert result.durable_webhook_jobs.failed_jobs >= 0
...
async def test_channel_prometheus_endpoint_uses_standard_content_type() -> None:
    ...
    assert b"openxflow_channel_webhook_job_pending" in response.body
    assert b"openxflow_channel_webhook_job_processing" in response.body
