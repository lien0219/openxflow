...
| `LANGFLOW_CHANNEL_WEBHOOK_QUEUE_TIMEOUT_SECONDS` | `0` | Maximum time an accepted callback may wait for an execution slot. `0` disables queue timeout and preserves indefinite waiting. |
...
export LANGFLOW_CHANNEL_WEBHOOK_QUEUE_TIMEOUT_SECONDS=0
...
2. After validation, it atomically reserves both queue-count capacity and retained-payload byte capacity before returning the provider acknowledgement. Workflow execution, file download, knowledge-base ingestion, and the provider reply run in an isolated database session with bounded queue and execution timing.
...
When a client disconnects before the streamed request body is complete, OpenXFlow returns HTTP `400` where the transport still permits a response. The partial body is discarded before adapter construction, signature verification, queue reservation, or background execution. The event increments `client_disconnected_total` but does not increment accepted, rejected-capacity, succeeded, or failed counters.
...
Capacity rejections are classified exactly once as pending-only, payload-bytes-only, or both limits at the same time. The sum of those three counters equals the aggregate rejected-capacity counter.

`LANGFLOW_CHANNEL_WEBHOOK_QUEUE_TIMEOUT_SECONDS` controls only the wait to acquire a concurrency slot. The default `0` preserves indefinite queue waiting. A positive timeout releases the reservation when the wait expires, does not start workflow execution, does not increase the active-job count, and increments both `failed_total` and `queue_timed_out_total` because the provider has already received a successful acknowledgement.
...
The JSON response separates Stream configuration intent from actual process state. `streams_enabled` shows the parsed environment setting, while `stream_runtime` reports current Manager, Leader and managed-client counts together with cumulative synchronization errors, client connection errors, reconnect attempts, successful database synchronization cycles, and the Unix timestamp of the last successful synchronization. The response also includes queue and execution timeout configuration, active and queued jobs, retained payload bytes, capacity limits, accepted and aggregate rejected counts, the three mutually exclusive rejection-reason counters, succeeded, failed, queue-timed-out, externally cancelled, and client-disconnected counts, and the outbound retry policy.
...
- `openxflow_channel_stream_sync_errors_total`
