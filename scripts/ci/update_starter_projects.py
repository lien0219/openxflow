...
if os.environ.get("GITHUB_HEAD_REF") == "automation/channel-active-flow-selection":
    ...
    source = source.replace('    new_handle = """', '    new_handle = r"""', 1)
    source = source.replace('    methods = """', '    methods = r"""', 1)
    source = source.replace(
        '    run_command(f"uvx --quiet ruff check --fix --config pyproject.toml --extend-ignore EM101,TRY003 {joined}")',
        '    run_command(f"uvx --quiet ruff check --fix --config src/backend/base/langflow/channels/ruff.toml {joined}")',
        1,
    )
    source = source.replace(
        '    run_command(f"git push origin HEAD:{BRANCH}")',
        '    run_command("git push --force origin HEAD:automation/channel-active-flow-result")',
        1,
    )
    source = source.replace(
...
''',
    ...
    action_filter_start = dispatch_source.index(action_filter, commands_start, commands_end)
    write(
        path,
        dispatch_source[:action_filter_start]
        + "            if item.command\\n"
        + '            in {"/commands", "/current-flow", "/whoami", "/files", "/knowledge", "/status"}\\n'
